...
namespace w9 {
	...
	void SecureData::code(char key)
	{
		...
		auto fun = std::bind(converter, text, key, nbytes, Cryptor());
		std::thread t1(fun);
		std::thread t2(fun);
		std::thread t3(fun);
		std::thread t4(fun);
	...
	void SecureData::backup(const char* file) {
		...
		else
		{
			...
			std::ofstream fun(file, std::ios::binary);
	...
	void SecureData::restore(const char* file, char key) {
		...
		std::ifstream fun(file, std::ios::binary);
		if (!fun) {
			throw std::string("\n***Failed to open file***\n");
		}
		else {
			...
			fun.seekg(0, std::ios::beg);

		// TODO: - allocate memory here for the file content
			text = new char[nbytes + 1];

		// TODO: - read the content of the binary file
			fun.read(text, nbytes);
			text[nbytes] = '\0';
		}
		fun.close();
